...
function createWorkspace(): { backlogDir: string; workspaceRoot: string } {
  const workspaceRoot = fs.mkdtempSync(path.join(os.tmpdir(), "backlog-shim-"));
  const backlogDir = path.join(workspaceRoot, ".backlog");
  fs.mkdirSync(backlogDir);
  return { backlogDir, workspaceRoot };
}

describe("writeLocalShim", () => {
  it("creates an executable shim at <backlogDir>/bin/backlog", () => {
    const { backlogDir, workspaceRoot } = createWorkspace();
    const shimPath = writeLocalShim(backlogDir, workspaceRoot);
  ...
  it("supports a BACKLOG_DEV_DIST override", () => {
    const { backlogDir, workspaceRoot } = createWorkspace();
    const contents = fs.readFileSync(writeLocalShim(backlogDir, workspaceRoot), "utf8");
  ...
  it("falls back to <workspace>/packages/cli/dist/bin.js for the dev-tree case", () => {
    ...
    expect(contents).toContain(`WORKSPACE_DIST="${workspaceRoot}/packages/cli/dist/bin.js"`);
  });
