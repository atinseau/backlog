  ...
  <div class="modal" onclick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" tabindex={-1} onkeydown={(e) => { if (e.key === "Escape") onClose(); }}>
    ...
    <div class="content">
      ...
      {:else if cloudStatus}
        ...
        <!-- Only GitHub is wired on the cloud backend right now. Google
             returns an OmniAuth "Authentication passthru" 404 from
             /users/auth/google_oauth2 (no strategy registered) and Apple
             needs Sign-in-with-Apple service IDs that aren't set up.
             Both will reappear here once the backend ships them. -->
