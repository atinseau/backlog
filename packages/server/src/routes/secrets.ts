...
// Secrets endpoint — exposes the workspace's encrypted secrets store
...
// workspace dir which is already implicit from being able to reach
// this endpoint.
...
export function secretsRoutes(): Hono<AppEnv> {
  ...
  app.get("/secrets", (c) => {
    ...
    const keys = listSecretKeys(workspace.backlogDir);
    return c.json({
  ...
  app.put("/secrets/:key", async (c) => {
    ...
    setSecret(workspace.backlogDir, keyParsed.data, parsed.data.value);
  ...
  app.delete("/secrets/:key", (c) => {
    ...
    const keyParsed = allowedKeys.safeParse(keyParam);
    if (!keyParsed.success) return c.json({ error: "unknown_key" }, 400);
    deleteSecret(workspace.backlogDir, keyParsed.data);
  ...
  app.get("/secrets/:key/exists", (c) => {
    ...
    const keyParsed = allowedKeys.safeParse(keyParam);
    if (!keyParsed.success) return c.json({ error: "unknown_key" }, 400);
    return c.json({ set: hasSecret(workspace.backlogDir, keyParsed.data) });
  });
